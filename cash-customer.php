...
$query2= mysqli_query($con,"SELECT order_info.order_id,order_info.cust_id,order_info.billing_name,
SUM(order_info.total_amount) as total_amount,
SUM(order_info.pending_amount) as pending_amount ,
SUM(order_info.paid_amount) as paid_amount ,
order_info.order_date ,bill_info.invoice_id 
FROM order_info 
INNER JOIN bill_info ON order_info.order_id=bill_info.order_id 
GROUP BY bill_info.billing_name ORDER BY order_info.sn DESC");
...
<td > <a href="sale-order-details.php?code=<?php echo $b['cust_id']; ?>"><?php echo $b['billing_name']; ?></a> </td>
