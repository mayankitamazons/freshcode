...
if($action=='get')
    {
        ...
        $sql="SELECT vendor.v_code, vendor.v_name FROM vendor 
        INNER JOIN vendor_order ON 
        vendor_order.v_code=vendor.v_code GROUP BY vendor_order.v_code ASC";
         ...
         while($data = mysqli_fetch_assoc($result))
        {
              ...
              array_push($vendorList,array("code"=>$v_code,"name"=>$v_name));

        }
        
         echo json_encode(array("list"=>$vendorList));
    }
      
    
    else if($action=='getOder'){       
          

         $result=mysqli_query($con,"SELECT * FROM `vendor_order` where v_code='$code' ORDER BY sn DESC");
         $row = mysqli_num_rows($result);
         while($data = mysqli_fetch_assoc($result))
        {          
