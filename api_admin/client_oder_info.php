...
if($action=='get')
    {
        ...
        $sql="SELECT client.c_code, client.c_name FROM client 
        INNER JOIN client_order ON 
        client_order.c_code=client.c_code GROUP BY client_order.c_code ASC";
         ...
         while($data = mysqli_fetch_assoc($result))
        {
              ...
              array_push($clientList,array("name"=>$c_name,"code"=>$c_code));
         ...
         $result=mysqli_query($con,"SELECT * FROM `client_order` where c_code='$code' ORDER BY sn DESC");
         $row = mysqli_num_rows($result);
         while($data = mysqli_fetch_assoc($result))
        {          
