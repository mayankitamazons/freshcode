...
if($action=='get')
    {
        ...
        $sql="SELECT client.c_code, client.c_name FROM client INNER JOIN client_order ON client_order.c_code=client.c_code GROUP BY client_order.c_code ASC";
         ...
         while($data = mysqli_fetch_assoc($result))
        {
            ...
            //   $v_code=$data["c_code"];
              ...
              $c_code=$data["c_code"];
                     ...
                     $result_order_id_data= mysqli_query($con,"SELECT * FROM `client_order` WHERE `c_code`='$c_code' ORDER BY `sn` DESC");
   ...
   $result_client_data22= mysqli_query($con,"SELECT client.c_code,client.c_name ,client_order.order_status FROM client INNER JOIN client_order ON client.c_code=client_order.c_code  WHERE client_order.order_id='$od_id'");
         ...
         $od_status=$order_data222["order_status"];
              ...
              $result_order_data= mysqli_query($con,"SELECT   order_info.product_code,order_info.c_code,order_info.v_code,client_order.order_status ,order_info.order_id,order_info.qty,order_info.payment_method,client_order.amount,order_info.order_date,
              order_info.price,order_info.product_name,order_info.product_name FROM client_order INNER JOIN order_info ON order_info.c_code=client_order.c_code 
              WHERE order_info.c_code ='$c_code' AND 
              order_info.order_id='$od_id' ORDER BY order_info.order_date");
              ...
              $order_date=$order_data["order_date"];
                ...
                $result_client_data= mysqli_query($con,"SELECT client.c_code,client.c_name FROM client INNER JOIN client_order ON client.c_code=client_order.c_code WHERE client_order.order_id='$order_id'");
                   ...
                   $client_data = mysqli_fetch_assoc($result_client_data);
                    ...
                    "order_status"=>$order_status,"order_id"=>$order_id,"amount"=>$amount,"payment_method"=>$payment_method,"order_status"=>$order_status,
              "order_date"=>$order_date,"c_name"=>$c_name,"c_code"=>$c_code,"pcode"=>$product_code));
                ...
                array_push($response,array("c_name"=>$c_name,"c_code"=>$c_code,"orderId"=>$orderId));
               $orderId=array();
